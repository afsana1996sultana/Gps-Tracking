...
(function ($) {
    ...
    if ($("#newsletterForm").length > 0) {
        ...
        $('#newsletterForm').on("submit", function () {
            $('#newsletterFormSubmit').html('<i class="fa fa-refresh fa-spin"></i> Submit');
            var rule = {
                email: {
                    notEmpty: {
            ...
            var isValid = smValidator("newsletterForm", rule, 2);
            if (isValid) {
                ...
                $.ajax({
                    ...
                    data: formData,
                    success: function (response) {
                        $('#newsletterFormSubmit').html('Submit');
                        $("#newsletterForm [name='email']").val("");
                        $("#newsletterPopUp .popup_title").html(response.title);
    ...
    /** Subscribe   form **/
    if ($("#newsletterPopUpForm").length > 0) {
        $('#newsletterPopUpForm').on("submit", function () {
            $('#newsletterPopUpFormSubmit').html('<i class="fa fa-refresh fa-spin"></i> Submit');
            var rule = {
                email: {
                    notEmpty: {
                        message: "The email field is Required!"
            ...
            var isValid = smValidator("newsletterPopUpForm", rule, 2);
            ...
            if (isValid) {
                ...
                $.ajax({
                    ...
                    data: formData,
                    success: function (response) {
                        $(".newslatter-popup-item").fadeOut();
                        $('#newsletterPopUpFormSubmit').html('Submit');
    ...
    if ($(".subscriptionClosedForADay").length > 0) {
        $(".subscriptionClosedForADay").on('click', function () {
            $.ajax({
                type: 'get',
                url: url + "/subscription-close-for-a-day",
                success: function (response) {
                    //success
                    //console.log(response);
                },
    ...
    if ($(".offerClosedForADay").length > 0) {
        $(".offerClosedForADay").on('click', function () {
            $.ajax({
                type: 'get',
                url: url + "/offer-close-for-a-day",
                success: function (response) {
                    //success
                },
                error: function (err) {
